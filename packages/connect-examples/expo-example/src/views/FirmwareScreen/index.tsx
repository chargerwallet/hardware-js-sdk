...
function FirmwareUpdate({
  ...
  const [chargerwalletFeatures, setChargerwalletFeatures] = useState<ChargerwalletFeatures | undefined>(undefined);
  ...
  const loadChargerwalletFeatures = useCallback(() => {
    if (!sdk) return;
    sdk.getChargerwalletFeatures(selectDevice?.connectId).then(res => {
      if (res.success) {
        setChargerwalletFeatures(res.payload);
      } else {
        setChargerwalletFeatures(undefined);
      }
    });
  ...
  }, [loadChargerwalletFeatures, sdk, selectDevice?.connectId, selectDevice?.features]);
  ...
  const deviceFieldProviderValue = useMemo(
    ...
    [features, chargerwalletFeatures]
  );
