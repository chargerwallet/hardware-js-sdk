...
import { UI_EVENT, getDeviceType } from '@chargerwallet/hd-core';
import { isEmpty } from 'lodash';
import type { CoreApi, Features, Success, Unsuccessful } from '@chargerwallet/hd-core';
...
export function useRunnerTest<T>(config: RunnerConfig<T>) {
  ...
  const beginTest = useCallback(async () => {
    try {
      ...
      try {
        const chargerwalletFeatures = await SDK.getChargerwalletFeatures(connectId);
        // @ts-expect-error
        setRunningChargerWalletDeviceFeatures?.(chargerwalletFeatures.payload);
      } catch (error) {
        // ignore
      }
