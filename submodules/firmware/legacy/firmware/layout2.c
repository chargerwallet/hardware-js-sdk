...
#define CROWDFUND_URL "crowdfund.chargerwallet.com"
...
#if !EMULATOR && !CHARGERWALLET_MINI
  ...
  if (config_getHomescreen(homescreen, sizeof(homescreen))) {
    ...
    if (backup_only) {
      ...
      if (strlen(label)) {
        ...
        oledDrawStringCenterAdapter(OLED_WIDTH / 2, 72, "ChargerWallet Mini",
                                    FONT_STANDARD);
      }
    ...
    if (backup_only) {
      ...
      if (strlen(label)) {
        ...
        oledDrawStringCenterAdapter(OLED_WIDTH / 2, 24, "ChargerWallet Mini",
                                    FONT_STANDARD);
      }
#endif
      if (!config_isInitialized()) {
...
#if CHARGERWALLET_MINI
  layoutTransInformation(NULL, NULL, &bmp_btn_cancel, &bmp_btn_confirm,
                         _("Cancel"), _("Confirm"), line1, line2, extra_line,
                         false, false, address);
#else
...
void layoutDialogAdapterEx(const BITMAP *icon, const char *btnNo,
                           ...
                           const char *line3, const char *line4,
                           const char *line5, const char *line6, uint8_t font) {
  const struct font_desc *font_dc = find_cur_font();
  ...
  if (line1) {
    oledDrawStringCenterAdapter(OLED_WIDTH / 2, 4 * (font_dc->pixel + 1), line1,
                                font);
  }
  if (line2) {
    oledDrawStringCenterAdapter(OLED_WIDTH / 2, 5 * (font_dc->pixel + 1), line2,
                                font);
  }
  if (line3) {
    oledDrawStringCenterAdapter(OLED_WIDTH / 2, 6 * (font_dc->pixel + 1), line3,
                                font);
  }
  if (line4) {
    oledDrawStringCenterAdapter(OLED_WIDTH / 2, 7 * (font_dc->pixel + 1), line4,
                                font);
  }

  if (desc) {
    oledDrawStringCenterAdapter(
        OLED_WIDTH / 2, OLED_HEIGHT - 2 * (font_dc->pixel + 1) - 1, desc, font);
    if (btnYes || btnNo) {
      oledHLine(OLED_HEIGHT - 2 * (font_dc->pixel + 1) - 3);
    }
  } else {
    if (line5) {
      oledDrawStringCenterAdapter(OLED_WIDTH / 2, 8 * (font_dc->pixel + 1),
                                  line5, font);
    }
    if (line6) {
      oledDrawStringCenterAdapter(OLED_WIDTH / 2, 9 * (font_dc->pixel + 1),
                                  line6, font);
    }
    if (btnYes || btnNo) {
      oledHLine(OLED_HEIGHT - (font_dc->pixel + 4));
    }
  }
  if (btnNo) {
...
void _layout_iterm_select(int x, int y, const BITMAP *bmp, const char *text,
  ...
  if (bmp) {
    ...
    oledDrawString(x - l / 2, y, text, font);
    if (vert) {
      oledInvert(x - l / 2, y - 1, x + l / 2 + 1, y + 8);
...
void layoutInputPassphrase(const char *text, uint8_t prefix_len,
  ...
  oledDrawString(1, 1, index_str, FONT_SMALL);
  ...
  oledInvert(icon_x, 0, icon_x + l + 1, 6);
  oledClearPixel(icon_x, 0);
  oledClearPixel(icon_x, 6);
  oledClearPixel(icon_x + l + 1, 0);
  oledClearPixel(icon_x + l + 1, 6);
...
#if CHARGERWALLET_MINI
  oledInvert(x - 2 + MINI_ADJUST, y1 - 1, x + l + 2, y1 + cur_font->pixel);
  oledClearPixel(x - 2 + MINI_ADJUST, y1 - 1);
  oledClearPixel(x - 2 + MINI_ADJUST, y1 + cur_font->pixel);
...
void layoutItemsSelectAdapterEx(
  ...
  if (!center_align) {
    ...
    oledInvert(0, (index + 1) * step + 1, OLED_WIDTH, (index + 2) * step - 2);
  } else {
    if (y == desc_pos)
      str = line1;
    else
...
void layoutItemsSelectAdapterAlign(const BITMAP *bmp_up, const BITMAP *bmp_down,
  ...
  if (!left_align) {
    ...
    switch (pos) {
      ...
      case 4:
      case 5:
        y1 += (cur_font->pixel + step) * 2;
        break;
      case 6:
      case 7:
        y1 += (cur_font->pixel + step) * 1;
        break;
      default:
        break;
    }
...
void layoutTransInformation(const BITMAP *bmp_up, const BITMAP *bmp_down,
  ...
  oledHLine(15);
  oledDrawStringCenterAdapter(OLED_WIDTH / 2, 3, title, FONT_STANDARD);
  if (bmp_up) {
    oledDrawBitmap(62, y, bmp_up);
  }

  y += steps;

  oledDrawStringCenterAdapter(OLED_WIDTH / 2, y, title_sub, FONT_STANDARD);
  y += steps;

  if (!qr_code) {
    if (center_align) {
      oledDrawStringCenterAdapter(OLED_WIDTH / 2, y, data, FONT_FIXED);
    } else {
  ...
  oledDrawStringCenterAdapter(OLED_WIDTH / 2, OLED_HEIGHT - 26, desc,
                              FONT_STANDARD);
  if (bmp_down) {
    oledDrawBitmap(62, OLED_HEIGHT - 11, bmp_down);
  }
...
#if !CHARGERWALLET_MINI && !EMULATOR
  char *se_version = NULL;
  char *se_sn;
#endif
...
refresh_menu:
  ...
  switch (index) {
    case 0:
      ...
      y += font->pixel + 1;
      oledDrawString(0, y, "ChargerWallet Mini", FONT_FIXED);

      y += font->pixel + 1;
      y += font->pixel + 1;
      oledDrawStringAdapter(0, y, _("FIRMWARE:"), FONT_STANDARD);
      y += font->pixel + 1;
      oledDrawStringAdapter(0, y, CHARGERWALLET_VERSION, FONT_FIXED);
#else
      if (se_get_sn(&se_sn)) {
    ...
    case 1:
      ...
      strcat(desc, "-");
      strcat(desc, CHARGERWALLET_VERSION);
      oledDrawString(0, y, "ST:", FONT_STANDARD);
      y += font->pixel + 1;
      oledDrawString(0, y, desc, FONT_FIXED);
      y += font->pixel + 1;
      y += font->pixel + 1;
      ...
      oledDrawBitmap((OLED_WIDTH - bmp_btn_down.width) / 2, OLED_HEIGHT - 8,
                     &bmp_btn_down);
#else
      layouKeyValue(y, _("Firmware version:"), CHARGERWALLET_VERSION);
