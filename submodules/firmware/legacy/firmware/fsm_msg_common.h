...
bool get_features(Features *resp) {
  ...
  if (device_is_factory_mode()) {
    ...
    strlcpy(resp->vendor, "chargerwallet.com", sizeof(resp->vendor));
    resp->has_model = true;
...
#endif
  {
    ...
    if (se_get_sn(&serial)) {
      resp->has_chargerwallet_serial = true;
      strlcpy(resp->chargerwallet_serial, serial, sizeof(resp->chargerwallet_serial));
    }
#endif
    ...
    resp->has_chargerwallet_version = true;
    strlcpy(resp->chargerwallet_version, CHARGERWALLET_VERSION, sizeof(resp->chargerwallet_version));
...
void fsm_msgSESignMessage(const SESignMessage *msg) {
  ...
  layoutDialogSwipeCenterAdapterEx(
      ...
      _("Check this device with\nChargerWallet secure server?"), NULL, NULL, NULL,
      NULL, NULL, NULL, NULL);
  if (!protectButton(ButtonRequestType_ButtonRequest_ProtectCall, false)) {
    fsm_sendFailure(FailureType_Failure_ActionCancelled, NULL);
    layoutHome();
